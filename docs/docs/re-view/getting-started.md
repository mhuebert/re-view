...
**`defview`** is a macro that returns a [view function](view-elements-components). Similar to `defn`, it expects a name, optional docstring, and arguments vector, followed by the body of the view, which should return valid [Hiccup-Syntax](../hiccup/syntax) or a React element.
