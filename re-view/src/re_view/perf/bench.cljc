(ns re-view.perf.bench
  (:require #?(:cljs ["benchmark" :as bench-js])
            [applied-science.js-interop :as j])
  #?(:cljs (:require-macros re-view.perf.bench)))

#?(:cljs
   (do
     (def Suite bench-js/Suite)
     (defn pad-right
...
#?(:clj
   ...
   (defmacro measure [title & kvs]
     (let [[title kvs] (if (string? title) [title kvs] [nil (cons title kvs)])]
       (assert (even? (count kvs)))
       (let [variants (apply hash-map kvs)
             ks (mapv first (partition 2 kvs))]
         `(let [suite# (new ~'re-view.perf.bench/Suite)
                ks# ~ks
                fns# ~(reduce-kv (fn [out k v] (assoc out k `(fn [] ~v))) {} variants)]
            ~(when title `(println (str "\nMeasuring: " ~title)))

            (doseq [[k# f#] fns#]
              (j/call suite# :add (~'re-view.perf.bench/pad-right (name k#) 15) f#))

            (j/call suite# :on "cycle"
                           (fn [event#]
                             (println (str (j/get event# :target)))))
            (j/call suite# :run)
            (when-not (apply = (map (comp cljs.core/js->clj #(%)) (vals fns#)))
              (print (map #(%) ((apply juxt ks#) fns#))))))))

   #_(defmacro measure [title repetitions & kvs]
